...
def main():
    ...
    result = db.client.table("simplefin_transactions").select("id, description, amount, category_id, subcategory_id, posted_date").order("posted_date", desc=True).limit(50).execute()
    ...
    if categorized:
        ...
        for t in categorized[:10]:  # Show first 10
            ...
            print(f"  {date} | ${t['amount']:>8.2f} | {t['description'][:40]:<40} | Cat: {t['category_id'][:8]}...")
